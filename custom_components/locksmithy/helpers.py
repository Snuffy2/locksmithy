...
if TYPE_CHECKING:
    from .lock import LockSmithyLock
...
class LockSmithyTimer:
    """Timer to use in LockSmithy."""

    def __init__(self) -> None:
        """Initialize the LockSmithy Timer."""
        self.hass: HomeAssistant | None = None
        self._unsub_events: list[Callable] = []
        self._lslock: LockSmithyLock | None = None
        self._call_action: Callable | None = None
        self._end_time: dt | None = None

    async def setup(
        self, hass: HomeAssistant, lslock: LockSmithyLock, call_action: Callable
    ) -> None:
        """Create fields for the LockSmithy Timer."""
        self.hass = hass
        self._lslock = lslock
    ...
    async def start(self) -> bool:
        """Start a timer."""
        if not self.hass or not self._lslock or not self._call_action:
            _LOGGER.error("[LockSmithyTimer] Cannot start timer as timer not setup")
            return False
        ...
        if sun.is_up(self.hass):
            delay: int = (self._lslock.autolock_min_day or DEFAULT_AUTOLOCK_MIN_DAY) * 60
        else:
            delay = (self._lslock.autolock_min_night or DEFAULT_AUTOLOCK_MIN_NIGHT) * 60
        self._end_time = dt.now().astimezone() + timedelta(seconds=delay)
        _LOGGER.debug(
            "[LockSmithyTimer] Starting auto-lock timer for %s seconds. Ending %s",
            int(delay),
            self._end_time,
        )
    ...
    async def cancel(self, timer_elapsed: dt | None = None) -> None:
        """Cancel a timer."""
        if timer_elapsed:
            _LOGGER.debug("[LockSmithyTimer] Timer elapsed")
        else:
            _LOGGER.debug("[LockSmithyTimer] Cancelling auto-lock timer")
    ...
    def is_setup(self) -> bool:
        ...
        if isinstance(self._end_time, dt) and self._end_time >= dt.now().astimezone():
            if isinstance(self._unsub_events, list):
                ...
                self._unsub_events = []
            self._end_time = None
        return bool(self.hass and self._lslock and self._call_action)
...
def _async_using(
    hass: HomeAssistant,
    domain: str,
    lslock: LockSmithyLock | None,
    entity_id: str | None,
) -> bool:
    """Evaluate if device is using specified integration logic."""
    if not (lslock or entity_id):
        raise TypeError("Missing arguments")
    ent_reg = er.async_get(hass)
    if lslock and lslock.lock_entity_id:
        entity = ent_reg.async_get(lslock.lock_entity_id)
    elif entity_id:
...
def async_using_zwave_js(
    hass: HomeAssistant,
    lslock: LockSmithyLock | None = None,
...
async def delete_code_slot_entities(
    hass: HomeAssistant, locksmithy_config_entry_id: str, code_slot_num: int
) -> None:
    ...
    #     entity_registry, locksmithy_config_entry_id
    # )
    ...
    for prop in properties:
        entity_id: str | None = entity_registry.async_get_entity_id(
            ...
            platform=DOMAIN,
            unique_id=f"{locksmithy_config_entry_id}_{slugify(prop)}",
        )
        if entity_id:
            try:
