...
class LockSmithyEntity(CoordinatorEntity[LockSmithyCoordinator]):
    """Base entity for LockSmithy."""

    def __init__(self, entity_description: LockSmithyEntityDescription) -> None:
        """Initialize base LockSmithy entity."""
        # _LOGGER.debug("[Entity init] entity_description: %s", entity_description)
        self.hass: HomeAssistant = entity_description.hass
        self.coordinator: LockSmithyCoordinator = entity_description.coordinator
        self._config_entry: ConfigEntry = entity_description.config_entry
        self.entity_description: LockSmithyEntityDescription = entity_description
        ...
        self._lslock: LockSmithyLock | None = self.coordinator.sync_get_lock_by_config_entry_id(
            self._config_entry.entry_id
        )
        if self._lslock:
            self._attr_name: str | None = f"{self._lslock.lock_name} {self.entity_description.name}"
        # _LOGGER.debug(
...
class LockSmithyEntityDescription(EntityDescription):
    """Base LockSmithy Entity Description."""

    hass: HomeAssistant
    config_entry: ConfigEntry
    coordinator: LockSmithyCoordinator
