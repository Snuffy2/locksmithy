"""Create the lovelace file for a LockSmithy lock."""
...
async def generate_lovelace(
    ...
    """Create the lovelace file for the LockSmithy lock."""
    folder: str = hass.config.path("custom_components", DOMAIN, "lovelace")
    filename: str = f"{lslock_name}.yaml"
    ...
    mapped_badges_list: (
        ...
        locksmithy_config_entry_id=locksmithy_config_entry_id,
    ...
    lovelace: list[MutableMapping[str, Any]] = [
        {
            ...
            "max_columns": 4,
            "title": f"{lslock_name}",
            "path": f"locksmithy_{lslock_name}",
...
def delete_lovelace(hass: HomeAssistant, lslock_name: str) -> None:
...
async def _map_property_to_entity_id(
    ...
    """Update all the entities with the entity_id for the LockSmithy lock."""
    # _LOGGER.debug(
    #     f"[map_property_to_entity_id] locksmithy_config_entry_id: {locksmithy_config_entry_id}, "
...
async def _generate_child_code_slot_dict(code_slot_num: int) -> MutableMapping[str, Any]:
    """Build the dict for the code slot of a child LockSmithy lock."""
