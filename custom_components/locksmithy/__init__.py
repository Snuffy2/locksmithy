...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    if not updated_config.get(CONF_NOTIFY_SCRIPT_NAME):
        updated_config[CONF_NOTIFY_SCRIPT_NAME] = (
            f"locksmithy_{updated_config.get(CONF_LOCK_NAME)}_manual_notify"
        )
    ...
    device_registry.async_get_or_create(
        ...
        configuration_url="https://github.com/Snuffy2/locksmithy",
    ...
    try:
        await coordinator.add_lock(lslock=lslock)
    ...
    await generate_lovelace(
        hass=hass,
        lslock_name=config_entry.data[CONF_LOCK_NAME],
        locksmithy_config_entry_id=config_entry.entry_id,
        parent_config_entry_id=config_entry.data.get(CONF_PARENT_ENTRY_ID),
...
async def delete_coordinator(hass: HomeAssistant, _: dt) -> None:
    """Delete the coordinator if no more lslock entities exist."""
