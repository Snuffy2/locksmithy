"""LockSmithy migrate from helper entites to coordinator."""
...
async def migrate_2to3(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    _LOGGER.info("[migrate_2to3] Moving states from helpers into lslock")
    lslock: LockSmithyLock = await _migrate_2to3_create_lslock(config_entry=config_entry)
    ...
    for ent_id, prop in crosswalk_dict.items():
        ...
        if not ent:
            continue
        await _migrate_2to3_set_property_value(lslock=lslock, prop=prop, value=ent.state)
    _LOGGER.debug("[migrate_2to3] lslock: %s", lslock)
    hass.data.setdefault(DOMAIN, {})
    if COORDINATOR not in hass.data[DOMAIN]:
        coordinator: LockSmithyCoordinator = LockSmithyCoordinator(hass)
    ...
    try:
        await coordinator.add_lock(lslock=lslock, update=True)
...
async def _migrate_2to3_create_lslock(config_entry: ConfigEntry) -> LockSmithyLock:
...
async def _migrate_2to3_set_property_value(lslock: LockSmithyLock, prop: str, value: Any) -> bool:
    if "." not in prop:
        return False
